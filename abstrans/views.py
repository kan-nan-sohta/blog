...
def home(request):
    print("home OK")
    return render(request, 'abstrans/index.html', {'msg': '論文のDOIかSemantic ScholarのURLを入力してください'})
...
def search(request):
    ...
    if request.POST['doi']:
        ...
        return render(request, 'abstrans/index.html', {'msg': '指定されたDOI，またはURLが見つかりません'})
...
def exec_ajax(request):
    ...
    return render(request,"abstrans/next.html", {"data":data})
...
def abstrans(request, doi):
    ...
    return render(request, 'abstrans/abstrans.html', {'papers': papers, 'doi':doi})
